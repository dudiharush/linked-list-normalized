...
export function fromDataArray<T>(dataArray: T[] = []) {
  ...
  const nodeMap = dataArray.reduce((nodeMap, data) => {
    const node = createNode(data);
    nodeMap[node.id] = node;
    if (prevNodeId) {
      nodeMap[prevNodeId].nextNodeId = node.id;
    }
    prevNodeId = node.id;
    return nodeMap;
  }, {} as NodeMap<T>);
  return fromNodes(nodeMap);
}

export function fromNodes<T>(loadedNodeMap: NodeMap<T> = {}) {
